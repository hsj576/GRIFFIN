# accelerate launch -m --mixed_precision=bf16 train.main_griffin_2 --tmpdir /your-datasets-path/llama3_data/sharegpt_0_67999_mufp16 --cpdir /your-model-paths/GRIFFIN-llama3-2 --configpath /your-GRIFFIN-path/GRIFFIN/train/llama_3_instruct_8B_config.json
...
parser.add_argument('--project', type=str, default='griffin')
parser.add_argument('--basepath', type=str, default='/your-GRIFFIN-path/Meta-Llama-3-8B-Instruct')
#parser.add_argument('--basepath', type=str, default='/your-GRIFFIN-path/Llama-2-7b-chat-hf')
#parser.add_argument('--basepath', type=str, default="/your-GRIFFIN-path/vicuna-7b-v1.5")
#parser.add_argument('--basepath', type=str, default="/your-GRIFFIN-path/llama-2-13b-chat-hf")
parser.add_argument('--griffinpath', type=str, default="/your-GRIFFIN-path/GRIFFIN-llama3-1")

parser.add_argument('--configpath', type=str, default="config.json")
parser.add_argument('--lr', type=float, default=3e-5)
parser.add_argument('--bs', type=int, default=4)
parser.add_argument('--topk_train', type=int, default=3)
parser.add_argument('--forward_num_total', type=int, default=2)
...
from model.cnets_griffin_2 import Model
...
def compute_loss(target, target_p, predict, regress, loss_mask):
    ...
    vloss = criterion(regress, target)
    vloss = torch.sum(torch.mean(loss_mask * vloss, 2)) / (loss_mask.sum() + 1e-5)

    topk_mask = torch.topk(target_p, k=10, dim=2).indices
...
if args.griffinpath is not None: 
    ea_model_path = args.griffinpath
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            ...
            for forward_idx in range(args.forward_num_total):
                predict, regress = model(hidden_states, input_ids, attention_mask, q_hidden_states=q_hidden_states)
                if q_hidden_states is None:
                    q_hidden_states = torch.cat([hidden_states[:, :1, :], regress[:, :-1, :]], dim=1)[None, :, :, :]
                else:
                    new_q_hidden_states = torch.cat([q_hidden_states[-1][:, :1, :], regress[:, :-1, :]], dim=1)[None, :, :, :]
                    q_hidden_states = torch.cat([q_hidden_states, new_q_hidden_states], dim=0)
                    
                q_hidden_states = q_hidden_states.detach()
                vloss, ploss, topk_loss, out_head = compute_loss(target, target_p, predict, regress, loss_mask)
                ...
                with torch.no_grad():
                    topk_train = args.topk_train
                    _, predicted_topk = torch.topk(out_head, k=topk_train, dim=2)
                    input_ids_2 = torch.cat([input_ids[:,1:], input_ids[:,:1]], dim=-1)

                    loss_mask_2_temp = None
                    for i in range(topk_train):
                        predicted_i = predicted_topk[:,:,i]
                        loss_mask_i_temp = ((predicted_i == input_ids_2)).int().unsqueeze(-1)
                        if loss_mask_2_temp == None:
                            loss_mask_2_temp = loss_mask_i_temp
                        else:
                            loss_mask_2_temp = torch.bitwise_or(loss_mask_i_temp, loss_mask_2_temp)

                    if forward_idx > 0:
                        loss_mask_pre = loss_mask_all[forward_idx]
                        loss_mask_pre_2 = torch.cat([loss_mask_pre[:,-1:,:], loss_mask_pre[:,:-1,:]], dim=1)
                        loss_mask_2_temp = loss_mask_pre_2 * loss_mask_2_temp

                    loss_mask_all.append(loss_mask_2_temp)
                    loss_mask_new = loss_mask_2_temp * loss_mask_all[0]
    ...
    if (epoch + 1) % train_config["save_freq"] == 0:
        ...
        for batch_idx, data in enumerate(tqdm(test_loader)):
            with torch.no_grad():
                ...
                vloss, ploss, topk_loss, out_head = compute_loss(data["target"], target_p, predict, regress, loss_mask)

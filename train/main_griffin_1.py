# accelerate launch -m --mixed_precision=bf16 train.main_griffin_1 --tmpdir /your-datasets-path/llama3_data/sharegpt_0_67999_mufp16 --cpdir /your-model-paths/GRIFFIN-llama3-1 --configpath /your-GRIFFIN-path/GRIFFIN/train/llama_3_instruct_8B_config.json
...
parser.add_argument('--basepath', type=str, default='/your-model-paths/Meta-Llama-3-8B-Instruct')
#parser.add_argument('--basepath', type=str, default='/your-model-paths/Llama-2-7b-chat-hf')
#parser.add_argument('--basepath', type=str, default="/your-model-paths/vicuna-7b-v1.5")
#parser.add_argument('--basepath', type=str, default="/your-model-paths/llama-2-13b-chat-hf")
...
from model.cnets_griffin_1 import Model
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            ...
            rloss = criterion(predict, regress)
            rloss = torch.sum(torch.mean(loss_mask * rloss, 2)) / (loss_mask.sum()+1e-5)
            vloss = criterion(regress, data["target"])
            vloss = torch.sum(torch.mean(loss_mask * vloss, 2)) / (loss_mask.sum()+1e-5)
            plogp = target_p * out_logp
            ploss = -torch.sum(torch.sum(loss_mask * plogp, 2)) / (loss_mask.sum()+1e-5)
            loss = train_config["v_w"] * vloss + train_config["p_w"] * ploss # + 0.1 * rloss
            # loss.backward()
        ...
        if accelerator.is_main_process and ct != 0:
            logdict = {"train/lr": optimizer.optimizer.param_groups[0]["lr"], "train/vloss": vloss.item(),
                       "train/ploss": ploss.item(), "train/rloss": rloss.item(), "train/loss": loss.item(), "train/acc": cc / ct}

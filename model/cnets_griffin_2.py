...
class EMLP(nn.Module):
    ...
    def forward(self, x, g):
        down_proj = self.down_proj(
            self.act_fn(
                self.up_proj(
                    torch.cat((self.input_layernorm(x), self.input_layernorm(g)), dim=-1)
                    )
                )
            )
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Fully Connected
        residual = torch.cat((hidden_states, hidden_states), dim=-1) 
...
class Efc(nn.Module):
    def __init__(self, config):
        ...
        self.mlp = EMLP(config)
        self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)

    def forward(
            self,
            hidden_states: torch.Tensor, 
            input_embeddding: torch.Tensor,
    ) -> torch.FloatTensor:
        """
        Args:
            hidden_states (`torch.FloatTensor`): input to the layer of shape `(batch, seq_len, embed_dim)`
        ...
        # Fully Connected
        hidden_states = self.fc(torch.cat((input_embeddding, hidden_states), dim=-1))
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states = self.mlp(hidden_states, input_embeddding)
        hidden_states = residual + hidden_states
...
class Model(nn.Module):
    ...
    def forward(
        ...
        if q_hidden_states is not None:
            q_hidden_states = self.fc(q_hidden_states, inputs_embeds[None, :, :, :].expand(q_hidden_states.shape[0], -1, -1, -1))
        ...
        if use_cache:
            return hidden_states[:,:,:self.hidden_size], hidden_states[:,:,self.hidden_size:], next_decoder_cache

        return hidden_states[:,:,:self.hidden_size], hidden_states[:,:,self.hidden_size:]
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        if hasattr(self, "stable_kv") and self.stable_kv is not None:
            ...
            out_hidden, out_regress, past_key_values = self(hidden_states, input_ids=input_ids[:, kv_len:],
                                            past_key_values=self.stable_kv, use_cache=True)
        else:
            out_hidden, out_regress, past_key_values = self(hidden_states, input_ids=input_ids, use_cache=True)
        ...
        input_hidden = out_regress[:, -1][None].repeat(1, top_k, 1)
        ...
        for i in range(depth):
            ...
            out_hidden, out_regress, past_key_values = self(input_hidden, input_ids=input_ids, past_key_values=past_key_values,
            ...
            input_hidden = out_regress[:, out_ids]
...
if __name__ == "__main__":
    ...
    model = Model(config, load_emb=True, path="/model/")
    print(model)

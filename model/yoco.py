...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            query_states = [F.linear(cross_hidden_states, query_slices[i]) for i in range(self.config.pretraining_tp)]
            query_states = torch.cat(query_states, dim=-1)
            ...
            query_states = self.q_proj(cross_hidden_states)
            key_states = self.k_proj(hidden_states)
            value_states = self.v_proj(hidden_states)
...
class LlamaCrossDecoderLayer(nn.Module):
    def __init__(self, config, index):
        ...
        self.cross_mlp = LlamaMLP(config)
        self.index = index
        if self.index != 0:
            self.cross_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.input_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.cross_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
            ...
            position_ids: Optional[torch.LongTensor] = None,
            past_key_value: Optional[Tuple[torch.Tensor]] = None,
            cross_key_value: Optional[Tuple[torch.Tensor]] = None,
        ...
        # Cross Attention
        hidden_states, cross_attn_weights, cross_key_value = self.cross_attn(
        ...
        cross_outputs = (hidden_states,)

        if output_attentions:
            cross_outputs += (cross_attn_weights,)

        if use_cache:
            cross_outputs += (cross_key_value,)
...
class YOCO(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0):
        ...
        self.layers = LlamaCrossDecoderLayer(config, 0)
    ...
    def forward(
            ...
            cross_key_values: Optional[List[torch.FloatTensor]] = None,
        ...
        cross_key_value = cross_key_values[0] if cross_key_values is not None else None
        ...
        if self.gradient_checkpointing and self.training:
            ...
            def create_custom_forward(module):
                def custom_forward(*inputs):
                    ...
                    return module(*inputs, past_key_value, cross_key_value, output_attentions)

                return custom_forward

            self_outputs, layer_outputs = torch.utils.checkpoint.checkpoint(
                create_custom_forward(self.layers),
                inputs_embeds,
                hidden_states,
                attention_mask,
                position_ids,
            ...
            self_outputs, layer_outputs = self.layers(
                inputs_embeds,
                hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
        ...
        if use_cache:
            next_cross_cache += (layer_outputs[2 if output_attentions else 1],)
            next_decoder_cache += (self_outputs[2 if output_attentions else 1],)

        if use_cache:
            return hidden_states, next_decoder_cache, next_cross_cache

# python -m application.webui --ea-model-path /your-model-paths/GRIFFIN-llama3-instruct-8B --base-model-path /your-model-paths/Meta-Llama-3-8B-Instruct --model-type llama-3-instruct
import os
...
def warmup(model):
    ...
    for output_ids, _, _ in model.ea_generate(input_ids):
...
def bot(history, temperature, top_p, use_EaInfer, highlight_EaInfer,session_state,):
    ...
    if use_EaInfer:
        ...
        for output_ids, _, _ in model.ea_generate(input_ids, temperature=temperature, top_p=top_p,
...
parser.add_argument(
    "--ea-model-path",
    type=str,
    default="/your-model-paths/GRIFFIN-llama3-instruct-8B",
    help="The path to the weights. This can be a local folder or a Hugging Face repo ID.",
)
parser.add_argument("--base-model-path", type=str, default="/your-model-paths/Meta-Llama-3-8B-Instruct",
...
with gr.Blocks(css=custom_css) as demo:
    ...
    with gr.Row():
        with gr.Column():
            use_EaInfer = gr.Checkbox(label="Use GRIFFIN", value=True)
            highlight_EaInfer = gr.Checkbox(label="Highlight the tokens generated by GRIFFIN", value=True)
